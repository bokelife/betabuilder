...
class ListBuilder extends Builder{
     ...
     * @param  [type] $placeholder 替换文本
     * @param  string $url 提交地址
     * @author Johnny <johnnycaimail@yeah.net>
     */
    public function setSearch($type='basic',$placeholder='请输入关键字', $url='') {
        if(!$url){
            $url = url($this->request->module().'/'.$this->request->controller().'/'.$this->request->action());
        }
        $this->search = ['type'=>$type,'placeholder' => $placeholder, 'url' => $url];
        return $this;
    }
    ...
    public function addBtn($position='top',$type, $attribute = null)
    {
        ...
        switch ($type) {
            ...
            case 'resume':  // 添加启用按钮(禁用的反操作)
                ...
                $my_attribute['class'] = 'btn btn-success js-ajax-dialog-btn btn-sm';
                $my_attribute['confirm-info'] = '您确定要执行启用操作吗？';
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus', ['status' => 'resume']) : url($this->request->module() . '/' . $this->request->controller() . '/setStatus', ['status' => 'resume']);
            ...
            case 'forbid':  // 添加禁用按钮(启用的反操作)
                ...
                $my_attribute['class'] = 'btn btn-warning js-ajax-dialog-btn btn-sm';
                $my_attribute['confirm-info'] = '您确定要执行禁用操作吗？';
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus', ['status' => 'forbid']) : url($this->request->module() . '/' . $this->request->controller() . '/setStatus', ['status' => 'forbid']);
                $my_attribute['href'] .= $query_model_params;
                break;
            case 'recycle':  // 添加回收按钮(还原的反操作)
                ...
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus', ['status' => 'recycle']) : url($this->request->module() . '/' . $this->request->controller() . '/setStatus', ['status' => 'recycle']);
                $my_attribute['href'] .= $query_model_params;
                break;
            case 'restore':  // 添加还原按钮(回收的反操作)
                ...
                $my_attribute['class'] = 'btn btn-success js-ajax-dialog-btn btn-sm';
                $my_attribute['confirm-info'] = '您确定要执行还原操作吗？';
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus', ['status' => 'restore']) : url($this->request->module() . '/' . $this->request->controller() . '/setStatus', ['status' => 'restore']);
                $my_attribute['href'] .= $query_model_params;
                break;
            case 'delete': // 添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                ...
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus', ['status' => 'delete']) : url(
    ...
    public function addRightButton($type, $attribute = null,$condition=[]) {
        ...
        switch ($type) {
            ...
            case 'forbid':  // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                ...
                $my_attribute['0']['href']  = $this->pluginName ? parent::plugin_url('setStatus',['status' => 'resume',$this->tablePrimaryKey => '__data_id__']) : url(
                        $this->request->module().'/'.$this->request->controller().'/setStatus',
                        array(
                            'status' => 'resume',
                            'ids' => '__data_id__',
                ...
                $my_attribute['1']['href']  = $this->pluginName ? parent::plugin_url('setStatus',['status' => 'forbid',$this->tablePrimaryKey => '__data_id__']) : url(
                        $this->request->module().'/'.$this->request->controller().'/setStatus',
                        array(
                            'status' => 'forbid',
                            'ids' => '__data_id__',
            ...
            case 'hide':  // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示隐藏/显示
                ...
                $my_attribute['2']['href']  = $this->pluginName ? parent::plugin_url('setStatus',['status' => 'show',$this->tablePrimaryKey => '__data_id__']) : url(
                        $this->request->module().'/'.$this->request->controller().'/setStatus',
                        array(
                            'status' => 'show',
                            'ids' => '__data_id__',
                ...
                $my_attribute['1']['href']  = $this->pluginName ? parent::plugin_url('setStatus',['status' => 'show',$this->tablePrimaryKey => '__data_id__']) : url(
                        $this->request->module().'/'.$this->request->controller().'/setStatus',
                        array(
                            'status' => 'hide',
                            'ids' => '__data_id__',
            ...
            case 'recycle':
                ...
                $my_attribute['confirm-info'] = '您确定要执行回收操作吗？';
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus',['status'=>'recycle','ids' => '__data_id__']) :  url(
                    $this->request->module().'/'.$this->request->controller().'/setStatus',
                    array(
                        'status' => 'recycle',
                        'ids' => '__data_id__',
            ...
            case 'restore':
                ...
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus',['status'=>'restore','ids' => '__data_id__']) :  url(
                    $this->request->module().'/'.$this->request->controller().'/setStatus',
                    array(
                        'status' => 'restore',
                        'ids' => '__data_id__',
            ...
            case 'delete':
                ...
                $my_attribute['class'] = $this->rightButtonType==1 ? 'btn btn-danger btn-xs js-ajax-dialog-btn':'ajax-get confirm';
                $my_attribute['confirm-info'] = '您确定要执行删除操作吗？';
                $my_attribute['href'] = $this->pluginName ? parent::plugin_url('setStatus',['status'=>'delete','ids' => '__data_id__']) :  url(
                    $this->request->module().'/'.$this->request->controller().'/setStatus',
                    array(
                        'status' => 'delete',
                        'ids' => '__data_id__',
    ...
    public function fetch($template_name='listbuilder',$vars ='', $replace ='', $config = '') {
        ...
        if ($this->request->isAjax()) {
            ...
            $builderConfig = parent::getConfig();
            $templateFile = $builderConfig['view_path'].$template_name.'.html';
            ...
            switch ($column_type) {
                ...
                case 'avatar':
                    if (!$data[$column['name']] || empty($data[$column['name']])) {
                        $data[$column['name']] = config('view_replace_str.__PUBLIC__').'/static/images/headicon.png';
                    }
                    $data[$column['name']] = '<img style="width:40px;height:40px;" src="'.cmf_get_user_avatar_url($data[$column['name']]).'">';
                    break;
                case 'image':
                    $pic_w = '120';
                ...
                case 'picture':
                    ...
                    $data[$column['name']] = '<img class="picture" width="'.$pic_w.'" src="'.cmf_get_image_url($data[$column['name']]).'">';
                    break;
                case 'pictures':
                    $pic_w = '120';
                    if (!empty($column['param']['width'])) {
                    ...
                    $data[$column['name']] = '<img class="pictures" width="'.$pic_w.'" src="'.cmf_get_image_url($temp[0]).'">';
                    break;
                case 'url'://以URL形式添加
                    $column_extra_attr = '';
